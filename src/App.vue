...
const remainingWords = computed(() => words.value.filter(word => word.memoryCount < 4).length);
const masteredWords = computed(() => words.value.filter(word => word.memoryCount >= 4).length);
const currentBatch = computed(() => {
  if (words.value.length === 0) return 0;
  return Math.floor((totalWords.value - remainingWords.value) / totalWords.value * 100);
});
...
const handleWordsLoaded = (loadedWords) => {
  ...
  if (savedProgress) {
    try {
      ...
      words.value = loadedWords.map(word => {
        ...
        return savedWord ? { ...word, memoryCount: savedWord.memoryCount } : { ...word, memoryCount: 0 };
      });
    } catch (e) {
    ...
    // 合并加载的单词和保存的进度
    words.value = loadedWords.map(word => {
      const savedWord = progressData.find(saved => 
  ...
  // 找到第一个未掌握的单词
  const firstNotMasteredIndex = words.value.findIndex(word => word.memoryCount < 4);
